...
function printFPS() {  
    ...
    c=$(awk "BEGIN{print(1000.0*$frame/$time)}")
    echo ", fps = "$c"(/s)"  
}
...
#lda_mat=$dir/lda.mat
# ext_lda_dim=$lda_dim
  ext_feat_dim=$(($feat_dim*$(($splice_width+$splice_width+1))))
  echo $ext_feat_dim
...
AffineComponentPreconditioned input-dim=$ext_feat_dim output-dim=$hidden_layer_dim alpha=$alpha max-change=$max_change learning-rate=$initial_learning_rate param-stddev=$stddev bias-stddev=$bias_stddev
...
while [ $x -lt $num_iters ]; do
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    #$cmd $dir/log/compute_prob_valid.$x.log \
      ...
      echo "epoch "$[$x/$iters_per_epoch]" ---> iter "$x" :" >> $dir/log/compute_prob_valid.log 
      nnet-compute-prob $dir/$x.mdl ark:$egs_dir/valid_diagnostic.egs >> $dir/log/compute_prob_valid.log &
    #$cmd $dir/log/compute_prob_train.$x.log \
      #nnet-compute-prob $dir/$x.mdl ark:$egs_dir/train_diagnostic.egs &
      echo  "epoch "$[$x/$iters_per_epoch]" ---> iter "$x" :" >> $dir/log/compute_prob_train.log 
      nnet-compute-prob $dir/$x.mdl ark:$egs_dir/train_diagnostic.egs >> $dir/log/compute_prob_train.log &
    if [ $x -gt 0 ] && [ ! -f $dir/log/mix_up.$[$x-1].log ]; then
      #$cmd $dir/log/progress.$x.log \
	#nnet-show-progress --use-gpu=no $dir/$[$x-1].mdl $dir/$x.mdl ark:$egs_dir/train_diagnostic.egs &
      $cmd $dir/log/progress.log \
	nnet-show-progress --use-gpu=no $dir/$[$x-1].mdl $dir/$x.mdl ark:$egs_dir/train_diagnostic.egs &
    fi
    ...
    #$cmd $parallel_opts JOB=1:$num_jobs_nnet $dir/log/train.$x.JOB.log \
	 ...
	 #--minibatch-size=$minibatch_size --srand=$x --gpu-id=JOB "$mdl" \
	#ark:- $dir/$[$x+1].JOB.mdl \
      #|| exit 1;

    #numjob=$(($num_jobs_nnet+1))
    #for((job=1;job<$numjob;job++))
    #do
       #echo "hello"
       #echo "iter "$x" :" >> $dir/log/train.$job.log || exit 1
    #done 
    $cmd $parallel_opts JOB=1:$num_jobs_nnet $dir/log/train.JOB.log \
      nnet-shuffle-egs --buffer-size=$shuffle_buffer_size --srand=$x \
      ark:$egs_dir/egs.JOB.$[$x%$iters_per_epoch].ark ark:- \| \
      nnet-train$train_suffix \
	 --minibatch-size=$minibatch_size --srand=$x \
	 --gpu-id=JOB --iter-num=$x "$mdl" \
	ark:- $dir/$[$x+1].JOB.mdl \
      || exit 1;
    ...
    if $shrink && [ $[$x % $shrink_interval] -eq 0 ]; then
      ...
      nnet-am-fix $dir/$[$x+1].mdl $dir/$[$x+1].mdl 2>$dir/log/fix.log 
    fi
  ...
  printFPS $start $end $[$samples_per_iter*$x*$num_jobs_nnet] >> $dir/log/compute_prob_valid.log
  end=$(date +%s.%N)
  printFPS $start $end $[$samples_per_iter*$x*$num_jobs_nnet] >> $dir/log/compute_prob_train.log 

  ## sync here
  #echo "sync for receive"
  #mpirun --host $host_name,$slave_name multinode_synchronize

  ## receive from other node
  #echo "receive form slave"

  ## combine model

  ## send to other node

  ##echo "sync for next iter"
  ## sync here
