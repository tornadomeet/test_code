...
input_lang=/styx/home/hzchenhaibo/s5-5/lang/train
...
# train
dir=data-fmllr-tri3b-$feat_type/train
steps/make_fmllr_opt_feats.sh --nj 10 \
   $dir data_$feat_type/train $gmmdir $dir/_log $dir/_data $pro_feat || exit 1
...
false && \
{ # Pre-train the DBN
dir=exp/tri3b_pretrain-dbn_$feat_type
#(tail --pid=$$ -F $dir/_pretrain_dbn.log 2>/dev/null)&
#$cuda_cmd $dir/_pretrain_dbn.log \
  steps/pretrain_dbn.sh --hid-dim 500 --rbm-iter 3 data-fmllr-tri3b-$feat_type/train $dir || exit 1;
}
...
#false && \
{ # Train the MLP
dir=exp/tri3b_pretrain-dbn_dnn_$feat_type
ali=exp/tri2b_ali
feature_transform=exp/tri3b_pretrain-dbn_$feat_type/final.feature_transform
dbn=exp/tri3b_pretrain-dbn_$feat_type/6.dbn
...
steps/train_nnet.sh --feature-transform $feature_transform --dbn $dbn --hid-layers 0 --learn-rate 0.008 \
 data-fmllr-tri3b-$feat_type/train_tr90 data-fmllr-tri3b-$feat_type/train_cv10 $input_lang $ali $ali $dir || exit 1;
...
dir=exp/tri3b_pretrain-dbn_dnn_fbank_smbr
srcdir=exp/tri3b_pretrain-dbn_dnn_fbank
acwt=0.1
...
steps/align_nnet.sh --nj 20 \
  data-fmllr-tri3b-fbank/train data/lang $srcdir ${srcdir}_ali || exit 1;
steps/make_denlats_nnet.sh --nj 20 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt $acwt \
  data-fmllr-tri3b-fbank/train data/lang $srcdir ${srcdir}_denlats  || exit 1;
}
# Now we re-train the DNN by 6 iterations of sMBR 
false && \
{
steps/train_nnet_mpe.sh --num-iters 6 --acwt $acwt --do-smbr true \
  data-fmllr-tri3b-fbank/train data/lang $srcdir ${srcdir}_ali ${srcdir}_denlats $dir || exit 1
}
# Decode
false && \
{
