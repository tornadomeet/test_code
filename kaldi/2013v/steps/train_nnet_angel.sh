...
#
hid_layers=       # nr. of hidden layers (prior to sotfmax or bottleneck)
...
# FEATURE PROCESSING
copy_feats=false  # resave the train features in the re-shuffled order to tmpdir
...
#  echo "feat-to-dim,firstly,num_fea=$num_fea"  # 792
  echo "dbn=$dbn"
...
#  echo "secondly,num_fea=$num_fea"  # 1024
  ...
  echo "the num_feat=$num_fea"
  echo "the num_tgt=$num_tgt"
  echo "the hid_layers=$hid_layers"
  echo "the hid_dim=$hid_dim"
#  num_fea=792
  # use make_nnet_proto.py to create a proto nnet
  utils/nnet/make_nnet_proto.py $proto_opts \
    ${bn_dim:+ --bottleneck-dim=$bn_dim} \
    $num_fea $num_tgt $hid_layers $hid_dim >$mlp_proto || exit 1
  # initialize
  # mlp_proto is the new added network without pre-training
  # and mpl_init here is initialized by mlp_proto
  mlp_init=$dir/nnet.init; log=$dir/log/nnet_initialize.log
  echo "before Initializing, the mlp_init=$mlp_init"
  echo "Initializing $mlp_proto -> $mlp_init"
  /styx/home/hzchenhaibo/sourcecode/kaldi-trunk-fast/src/nnetbin/nnet-initialize $mlp_proto $mlp_init 2>$log || { cat $log; exit 1; }
# nnet-initialize $mlp_proto $mlp_init 2>$log || { cat $log; exit 1; }
...
mlp_init=$dir/nnet/nnet_dnn.init
steps/train_nnet_scheduler_angel.sh \
