...
echo "input dim $num_fea "
echo "output dim $num_tgt "

mlp_proto=$dir/nnet.proto

for iter in $(seq -w $hid_layers); do

	if [ $iter -eq 1 ];then
		mlp_init=$dir/nnet_$iter.init
		utils/nnet/make_nnet_proto.py $proto_opts \
		${bn_dim:+ --bottleneck-dim=$iter} \
		$num_fea $num_tgt $iter $hid_dim >$mlp_proto.$iter || exit 1

		nnet-initialize $mlp_proto.$iter $mlp_init
	else

		mlp_init_old=$mlp_init
		mlp_init=$dir/nnet_$iter.init
		num_fea=1024
		num_hid=0
		...
		utils/nnet/make_nnet_proto.py $proto_opts \
		${bn_dim:+ --bottleneck-dim=$iter} \
		$num_fea $num_tgt $num_hid $hid_dim >$mlp_proto.$iter || exit 1

		nnet-initialize $mlp_proto.$iter $mlp_init
		nnet-remove-softmax $mlp_init_old $mlp_init_old

		nnet-concat $mlp_init_old $mlp_init $mlp_init
	fi

#steps/train_simple_nnet.sh
