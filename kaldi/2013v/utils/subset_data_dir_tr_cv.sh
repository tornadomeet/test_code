...
# Begin configuration.
cv_spk_percent=6 # % of speakers is parsed by option
cv_utt_percent=6 # default 10% of total utterances 
seed=777 # use seed for speaker shuffling
# End configuration.

echo "$0 $@"  # Print the command line for logging

uttbase=false; # by default, we choose last 10% utterances for CV
...
#awk '{print $1}' $srcdir/spk2utt > $trndir/_tmpf_randspk

#split the train/cv

#start_line=174059
#end_line=$((start_line+N_spk_cv))

#train_start=$((start_line-1))
#train_end=$((end_line+1))

#sed -n "$start_line,${end_line}p" $trndir/_tmpf_randspk > $cvdir/_tmpf_cvspk
#sed -n "1,${train_start}p" $trndir/_tmpf_randspk > $trndir/_tmpf_trainspk
#sed -n "${train_end},${N}p" $trndir/_tmpf_randspk >> $trndir/_tmpf_trainspk

head -n $N_spk_cv $trndir/_tmpf_randspk > $cvdir/_tmpf_cvspk
tail -n $N_spk_trn $trndir/_tmpf_randspk > $trndir/_tmpf_trainspk

#head -n $N_spk_trn $trndir/_tmpf_randspk > $trndir/_tmpf_trainspk
#tail -n $N_spk_cv $trndir/_tmpf_randspk > $cvdir/_tmpf_cvspk
